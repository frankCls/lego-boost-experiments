...
export const paperFeed = async () => {
  await move(10, Direction.DOWN);
}
...
export const move = async (inputDistance: number, extdirection: Direction) => {
  ...
  position[direction.axis] += inputDistance * motorDirection * (direction.axis === 'y' ? -1 : 1);
  ...
  console.log(`moving X: distance ${inputDistance} ${direction.value}`);
  logPosition();
}
...
export const run = (injectFunction: InjectFunction) => {
  poweredUP.on("discover", async (hub: MoveHub) => { // Wait to discover a Hub
    ...
    motorX = <TachoMotor>await hub.waitForDeviceAtPort(MOTOR_A);
    motorPen = <TachoMotor>await hub.waitForDeviceAtPort(EXTERNAL_MOTOR); // Make sure a motor is plugged into port A
    motorY = <TachoMotor>await hub.waitForDeviceAtPort(MOTOR_B); // Make sure a motor is plugged into port B
    motorXY = <TachoMotor>await hub.waitForDeviceAtPort(MOTOR_A_B);
