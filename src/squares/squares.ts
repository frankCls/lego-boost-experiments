...
import {motorPen, motorX, motorY} from "../names/hub";
...
poweredUP.on("discover", async (hub: MoveHub) => { // Wait to discover a Hub
  ...
  const motorX = <TachoMotor>await hub.waitForDeviceAtPort(EXTERNAL_MOTOR);
  const motorPen = <TachoMotor>await hub.waitForDeviceAtPort(MOTOR_A); // Make sure a motor is plugged into port A
  const motorY = <TachoMotor>await hub.waitForDeviceAtPort(MOTOR_B); // Make sure a motor is plugged into port B
   ...
   */
  const moveX = async (inputDistance: number, direction: -1 | 1) => {
  ...
  const moveY = async (inputDistance:number, direction: -1 | 1) => {
...
// while (true) { // Repeat indefinitely
  ...
  await pen()
  await pen()
  await moveX(10, RIGHT)
  await pen()
  await moveX(30, RIGHT)
  await pen()
  await moveX(10, RIGHT)
  process.exit();
